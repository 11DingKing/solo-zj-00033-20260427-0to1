<script lang="ts">
  ...
  async function loadSnippets(pageNum: number = 1) {
    loading = true;
    try {
      const response = await api.get(`/api/snippets/latest?page=${pageNum}&per_page=12`);
      if (response.ok) {
      ...
      console.error('Failed to load latest snippets:', e);
    } finally {
      loading = false;
    }
  }
...
<div class="list-page">
  <div class="page-header">
    ...
    <p class="subtitle">Most recently created snippets</p>
  </div>
